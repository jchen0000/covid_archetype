...
if __name__ == '__main__':
   ...
   # Load covid positive patient data
    ...
    count_los = unique_df.groupby('los').size()
    count_los = count_los.to_frame()
    ...
    # Entropy heatmap plot
    pd_se_lab_all = pd_se_lab_all.reset_index()
    ax = plt.gca()
    data_to_plot = pd_se_lab_all.loc[:, pd_se_lab_all.columns[3:]]
    fig = ax.imshow(data_to_plot.to_numpy().astype('float').T, aspect=0.2)  # delete the first row
